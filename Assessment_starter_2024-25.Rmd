...
```{r}
table_1 <- data_march2024 %>% 
  select(bnf_item_description, number_of_paid_items, paid_quantity) %>% 
  group_by(bnf_item_description) %>% 
  summarise(total = sum(paid_quantity)) %>% 
  arrange(desc(total)) %>% 
  head(n = 10) %>% 
  gt() %>% 
  cols_label(bnf_item_description = "Medicine", total = "Total Prescribed") %>%
  cols_align(align = "center") %>% 
  tab_header(title = "Top 10 Most Prescribed Medicines in Scotland",
             subtitle = "March 2024") %>% 
  tab_source_note("Data from Public Health Scotland") %>% 
  ...
  tab_style(style = list(
    ...
    cell_text(style = "oblique", color = "white")),
...
```{r}
board_raw <- read_csv("https://www.opendata.nhs.scot/dataset/9f942fdb-e59e-44f5-b534-d6e17229cc7b/resource/652ff726-e676-4a20-abda-435b98dd7bdc/download/hb14_hb19.csv") %>% 
  clean_names()

named_data_march2024 = left_join(data_march2024, board_raw, by = c("hbt" = "hb"))
...
```{r}
#finding the different rows
unique(named_data_march2024$hbt)
#found SB0806 = Scottish Ambulance Service 
...
```{r}
joined_data = left_join(named_data_march2024, population_data, by = "hb_name")
...
```{r}
paracetamol_chart <-paracetamol_data %>% 
  filter(!str_detect(bnf_item_description, "ORAL SUSPENSION")) %>%
  group_by(hb_name) %>%
  summarise(total_paid = sum(paid_quantity)) %>% 
  left_join(., population_data, by = "hb_name") %>% 
  mutate(Caplets_per_person = total_paid/hb_population) %>% 
  ggplot(aes(x =fct_reorder(hb_name, Caplets_per_person), y = Caplets_per_person))+
    geom_col(aes(fill = fct_reorder(hb_name, Caplets_per_person)), show.legend = FALSE)+
  coord_flip()+
  labs(y = "Caplets per Person", x = "Health Board", title = "Caplets of Paracetamol per Person in Scottish Health Boards")
